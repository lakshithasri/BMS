...
{
    public class ExcelImportService(IUnitOfWork unitOfWork) : IExcelImportService
    {
        private readonly IUnitOfWork _unitOfWork = unitOfWork;
        public async Task<ImportResultDto> ImportExcelDataAsync(Stream excelStream, string fileName)
        {
        ...
        private List<ExcelImportRowDto> ParseExcelRows(IXLWorksheet worksheet)
        {
            ...
            for (int row = 2; row <= rowCount; row++)
            {
                var importRow = new ExcelImportRowDto
                {
                    ...
                    Property = GetCellValue(worksheet, row, 16),
        ...
        private async Task ProcessRowAsync(ExcelImportRowDto row)
        {
            ...
            var property = await GetOrCreatePropertyAsync(row.Property, row.Address, row.Suburb);

            // 2. Create or find Tenant
            var tenant = await GetOrCreateTenantAsync(row.TenantOwner);

            // 3. Create or find Tenancy
            var tenancy = await GetOrCreateTenancyAsync(property.Id, tenant.Id, row.Tenancy, row.Lease, row.Category);
        ...
        private async Task<Tenancy> GetOrCreateTenancyAsync(int propertyId, int tenantId, string tenancyRef, string leaseRef, string category)
        {
            var tenancy = await _unitOfWork.TenancyRepository
                .FirstOrDefaultAsync(t => t.PropertyId == propertyId && t.TenantId == tenantId && t.LeaseReference == leaseRef);
        ...
        private async Task<Meter> GetOrCreateMeterAsync(int propertyId,
            string meterName,
            string register,
            string unit,
            decimal multiplier)
        {
            var meter = await _unitOfWork.MeterRepository
                .FirstOrDefaultAsync(m => m.PropertyId == propertyId && m.MeterName == meterName && m.Register == register);
