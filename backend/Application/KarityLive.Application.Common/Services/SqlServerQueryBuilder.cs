...
namespace KlarityLive.Application.Features.Base.Services
{
    ...
    {
        public (string sql, object parameters) BuildGetByIdQuery<TEntity>(int id, string tableName) where TEntity : BaseEntity
        {
        ...
        public (string sql, object parameters) BuildGetAllQuery<TEntity>(GetAllQuery<TEntity> query, string tableName) where TEntity : BaseEntity
        {
            //var tableName = GetTableName<TEntity>();
            var columns = GetColumnNames<TEntity>();
        ...
        public (string sql, object parameters) BuildCountQuery<TEntity>(Dictionary<string, object> filters, string tableName) where TEntity : BaseEntity
        {
            //var tableName = GetTableName<TEntity>();
        ...
        //private string GetTableName<TEntity>(string ) where TEntity : class
        //{
        //    // Simple pluralization - you might want to use a more sophisticated approach
        //    var entityName = typeof(TEntity).Name;
        //    return entityName.EndsWith("y") ? $"[{schemaName}]." + entityName.Substring(0, entityName.Length - 1) + "ies" : entityName + "s";
        //}
